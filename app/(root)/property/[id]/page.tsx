...
const StaticPropertyMap = ({
  ...
  const staticMapUrl =
    `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=14&size=800x400&scale=2&format=png&maptype=roadmap&style=feature:all|element:labels|visibility:on&markers=color:red%7C${lat},${lng}&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`.replace(
      /\s/g,
      ""
    );
  ...
  const handleContactOwner = async () => {
    ...
    try {
      ...
      const channel = streamClient.channel("messaging", {
        members: [session.user.id, owner.id],
      ...
      router.push("/inbox");
      toast.success("Chat initiated with the property owner!");
    } catch (error) {
